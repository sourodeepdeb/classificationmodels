...
for smilesCol, prefix in smilesRoles:
    if smilesCol in data.columns:
        ...
        for i in range(fp_bits):
            ...
            if col in data.columns and col not in numericalFeatures:
                numericalFeatures.append(col)
        vectorizer = CountVectorizer(analyzer='char', ngram_range=(2, 4), min_df=1, max_features=16)
...
plt.title("Test Set: Actual vs Predicted Yield (Random Forest, Fast Params)")
plt.grid(True)
plt.tight_layout()
plt.show()

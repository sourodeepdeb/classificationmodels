...
fpBits = 128
numericalFeatures = ['Temperature', 'Time', 'TempTimeInteraction',
                     'COOH MW', 'COOH logP', 'Amine MW', 'Amine logP']
categoricalFeatures = ['Solvent', 'Coupling Agent', 'COOH', 'Amine', 'Additive',
                       'Amine SMILES', 'COOH SMILES']
...
baseEstimator = DecisionTreeRegressor()
...
search = RandomizedSearchCV(
    AdaBoostRegressor(estimator=baseEstimator, random_state=42),
...
bestAda = search.best_estimator_
yPred = bestAda.predict(xTest)
r2 = r2_score(yTest, yPred)
print(f"AdaBoost Regressor r^2: {r2}")

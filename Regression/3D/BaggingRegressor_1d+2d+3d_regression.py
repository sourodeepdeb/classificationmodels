...
for smilesCol, prefix in smilesRoles:
    if smilesCol in data.columns:
        ...
        fp_df = pd.DataFrame(fps.tolist(), columns=[f'{prefix}_FP_{i}' for i in range(fpBits)], index=data.index)
...
best_bagging = search.best_estimator_
yPred = best_bagging.predict(xTest)
r2 = r2_score(yTest, yPred)

print(f"Bagging Regressor r^2: {r2}")

...
from rdkit.Chem import Descriptors, AllChem, DataStructs, rdMolDescriptors, rdPartialCharges
...
def get_3d_features(mol):
    ...
    try:
        ...
        prop_values = props.ComputeProperties(mol)
        shadow = rdMolDescriptors.CalcSlogP_VSA(mol)
        mq = rdMolDescriptors.MQNs_(mol)
        estate = rdMolDescriptors.EStateIndices(mol)
        rdPartialCharges.ComputeGasteigerCharges(mol)
        charges = [atom.GetDoubleProp('_GasteigerCharge') for atom in mol.GetAtoms()]
        return list(prop_values) + [np.mean(shadow)] + list(mq[:5]) + [np.mean(estate)] + [np.mean(charges)]
    except:
        return [np.nan] * 15
...
for smiles_col, prefix in smiles_roles:
    ...
    desc_1d.columns = [f'{prefix}_MW', f'{prefix}_logP', f'{prefix}_HAcceptors', f'{prefix}_HDonors',
                      f'{prefix}_TPSA', f'{prefix}_RotBonds', f'{prefix}_RingCount']
    ...
    desc_3d.columns = [f'{prefix}_3D_{i}' for i in range(15)]

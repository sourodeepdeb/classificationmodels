...
print(f"r^2: {r2_score(yTrain, yPred_train)}")
print(f"r^2: {r2_score(yTest, yPred_test)}")

plt.figure(figsize=(10, 8))
plt.scatter(yTest, yPred_test, alpha=0.5, label='Predictions')
...
sorted_idx = np.argsort(np.abs(model.coef_))[::-1][:20]
plt.barh(range(20), np.abs(model.coef_[sorted_idx]), align='center')
